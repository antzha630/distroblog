...
class Database {
  ...
  async createTables() {
    ...
    try {
      ...
      // Add publisher_description column if it doesn't exist (for existing tables)
      await client.query(`
        ALTER TABLE articles 
        ADD COLUMN IF NOT EXISTS publisher_description TEXT
      `);

      // Add article_hook column if it doesn't exist (for existing tables)
      // This stores a concise one-liner hook generated by AI to help users quickly understand articles
      await client.query(`
        ALTER TABLE articles 
  ...
  async getArticlesByDateRange(days) {
    // Get articles where either pub_date (if present) OR created_at is within the last N days
    // This ensures newly discovered articles without explicit pub_date still appear in the dashboard
    const result = await this.pool.query(`
      SELECT a.*, s.name as source_name 
      FROM articles a 
      LEFT JOIN sources s ON a.source_id = s.id 
  ...
  async addArticle(article) {
    const result = await this.pool.query(`
      INSERT INTO articles (
        ...
        category, status, is_manual, ai_summary, publisher_description, article_hook
